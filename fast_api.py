...
# New API endpoints for Projects and Internships

@app.post("/get_projects")
def get_projects(request: SearchRequest):
    projects = []
    for interest in request.interests:
        params = {
            "q": f"{interest} site:github.com OR site:devpost.com",
            "api_key": SERPAPI_KEY,
            "engine": "google"
        }
        response = requests.get(API_URL, params=params)
...
@app.post("/get_internships")
def get_internships(request: SearchRequest):
    internships = []
    for interest in request.interests:
        params = {
            "q": f"{interest} internships site:internshala.com OR site:linkedin.com OR site:glassdoor.com",
            "api_key": SERPAPI_KEY,
            "engine": "google"
        }
        response = requests.get(API_URL, params=params)
        ...
        print("Response Data:", response.text)  # Debugging

        if response.status_code == 200:
            data = response.json()
            if not data.get("organic_results"):  # Check if results are empty
                print("No results found for", interest)
            for result in data.get("organic_results", []):
                internships.append({
                    "title": result.get("title"),
                    "link": result.get("link"),
